...
export class QuestionnaireComponent implements OnInit {
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.questionnaires = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: IQuestionnaire[] | null): IQuestionnaire[] {
    const questionnairesNew = this.questionnaires ?? [];
    if (data) {
      for (const d of data) {
        if (questionnairesNew.map(op => op.id).indexOf(d.id) === -1) {
          questionnairesNew.push(d);
        }
      }
    }
    return questionnairesNew;
  }
